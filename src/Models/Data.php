...
final class Data
{
    ...
    public function open(): void
    {
        if (!isset($this->pdo) || $this->pdo === null) {
            $this->pdo = new PDO(
                BD_CONFIG["DRIVER"] . ":host=" . BD_CONFIG["HOST"] . ";port=" . BD_CONFIG["PORT"] . ";dbname=" . BD_CONFIG["DBNAME"],
                BD_CONFIG["USERNAME"],
                BD_CONFIG["PASSWD"],
                BD_CONFIG["OPTIONS"]
            )
                or
            die(header($_SERVER["SERVER_PROTOCOL"] . 401 . " Not authorized"));
        }
    }
    ...
    public function add(string $table, string $columns,int $quantity): Data
    {
        try {
            ...
            if (empty($table) || empty($columns) || empty($quantity)) throw new Exception("null values", 400);
    ...
    {
        try {
            if (empty($table)) throw new Exception("null values", 400);
            if ($option <= 0 || $option > 6) throw new Exception("Value <mark>$option</mark> is not accepted", 400);
            if (!is_numeric($option)) throw new Exception("Non-numeric value", 4000);
    ...
    {
        try {
            if (empty($table) || empty($columns) || empty($where) ) throw new Exception("null values", 400);
    ...
    {
        try {
            if (empty($table) || empty($where) || empty($val)) throw new Exception("null values", 400);
